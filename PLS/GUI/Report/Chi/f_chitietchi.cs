...
{
    public partial class f_chitietchi : Form
    {
        ...
        private void danhmuc_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (danhmuc.Text == "Loại chi - ປະເພດ")
            {
                try
                {
                    var lst = (from a in db.dmpchis select new { id = a.danhmuc, name = a.danhmuc_l == null ? "---" : a.danhmuc_l, key = Biencucbo.donvi + a.danhmuc + Biencucbo.idnv });
            ...
            try
            {
                ...
                else
                {
                    ...
                    var lst = from a in lst6
                              where a.ngaychi >= DateTime.Parse(tungay.Text) && a.ngaychi <= DateTime.Parse(denngay.Text)
                              select a;
