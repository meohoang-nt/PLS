...
namespace GUI
{
    public partial class f_dspttoan : Form
    {
        ...
        public void loaddata(DateTime tungay, DateTime denngay)
        {
            ...
            var lst = from a in db.r_pttoans
                      join d in db.donvis on a.iddv equals d.id
                      where
                      a.ngaythu >= tungay && a.ngaythu <= denngay
                      select new
                      {
                          id = a.id,
                          ngaythu = a.ngaythu,
        ...
        private void f_PN_Load(object sender, EventArgs e)
        {
            ...
            this.Text = LanguageHelper.TranslateMsgString("." + Name + "_title", "Danh Sách Phiếu Thu Tiền Từ Xa").ToString();
